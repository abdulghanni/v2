...
class Himpunan_model extends CI_Model {
    ...
    public function himpunan_records_search($limit, $start) 
    {
        ...
        ->order_by('e.regyear desc, title desc');
        if($this->session->userdata('sess_title')!=null){
            $this->db->where('e.title', $this->session->userdata('sess_title'));
        }
        if($this->session->userdata('sess_category')!=null){
            $this->db->where('c.category_id', $this->session->userdata('sess_category'));
        }
         if($this->session->userdata('sess_year')!=null){
            $this->db->where('e.regyear', $this->session->userdata('sess_year'));
        }
        $query = $this->db->get();
    ...
    public function produk_hukum_total() 
    {
        $prod = $this->load->database('produkhukum', true);
        return $prod->count_all("entries");
    }
    ...
    public function himpunan_search_total() 
    {
        $this->db
        ->select('c.description as c_description, e.description as e_description, ')    
        ->select('e.entry_id, e.FK_category_id, e.FK_user_id, e.active, CAST(e.title as SIGNED) as title, e.regyear, e.date_added, e.url')
